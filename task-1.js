...
const getSortedUniqueSkills = users => {
  const allSkils = users.reduce((acc, curr) => {
    curr.skills.forEach((skill) => acc.push(skill));
    return acc;
  }, []);
// ----------------

  const filterSkills = allSkils.filter((skill, index, arrey) => {
    return arrey.indexOf(skill) === index;
  });
// ----------------

  return filterSkills.sort((a,b) => a.localeCompare(b));
};
